...
export default function Home() {
   ...
   const [idBond, setidBond] = useState('')
   const [order, setorder] = useState()
   const [created_at, setcreated_at] = useState()
   const [voucher, setvoucher] = useState()
   //   const [tablaBond, setTablaUsuarios]= useState([]);
   //   const [busqueda, fetchBusqueda]= useState("");

   const getData = () => {
      fetch('http://127.0.0.1:8000/order/my_bond/')
         .then((res) => res.json())
         .then((res) => {
            fetchBond(res)
            setFilterBond(res)
   ...
   const deleteBond = (idBond) => {
      setSearching(false)
      let rpta = window.confirm('¿Desea eliminar el pedido de bono?')
      if (rpta) {
         fetch('http://127.0.0.1:8000/order/my_bond/' + idBond + '/', { method: 'DELETE' })
            .then((res) => {
               console.log(res)
               var temp = Bond.filter((i) => i.id !== idBond);
               fetchBond(temp)
            }
   ...
   const handleChange = e => {
      ...
      results = Bond.filter((bond) => {
         if (bond.order.toString().toLowerCase().indexOf(e.target.value.toLowerCase()) >= 0
            || bond.created_at.toString().toLowerCase().indexOf(e.target.value.toLowerCase()) >= 0
         ) {
            return bond;
         }
      }
   ...
   //    var resultadosBusqueda=Bond.filter((bond)=>{
   //      if(bond.order.toString().toLowerCase().includes(terminoBusqueda.toLowerCase())
   //      || bond.created_at.toString().toLowerCase().includes(terminoBusqueda.toLowerCase())
   //      ){
   //        return bond;
   //      }
   //    });
   //    fetchBond(resultadosBusqueda);
   //  }
   ...
   const editBond = (e) => {
      ...
      console.log(datos);
      axios.put('http://127.0.0.1:8000/order/my_bond/' + idBond + '/', datos)
         .then(res => {
            Bond.push(datos);
            setidBond('')
            setorder('')
                                          ...
                                          <div class="relative py-6 px-4 w-full max-w-md h-full md:h-auto">
                                             ...
                                             <div class="relative bg-white rounded-lg shadow">
                                                ...
                                                <form class="px-6 pb-4 space-y-6 lg:px-8 sm:pb-6 xl:pb-8" action="#">
                                                   <h3 class="text-center text-xl font-medium text-gray-900 dark:text-white">Ingrese los nuevos datos del pedido de Bono</h3>
                                                   ...
                                                   <button onClick={editBond} class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Editar pedido de bono</button>
